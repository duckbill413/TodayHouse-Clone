...
public class CustomSocialLoginSuccessHandler implements AuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException, ServletException {
        log.info("CustomSocialLoginSuccessHandler : " + authentication.getPrincipal());
        ...
        usersSecurityDTO.getAuthorities().stream().collect(Collectors.toList()).forEach(grantedAuthority -> {
            UserRole userRole = Enum.valueOf(UserRole.class, grantedAuthority.toString().substring(5));
            roles.add(userRole);
        });
